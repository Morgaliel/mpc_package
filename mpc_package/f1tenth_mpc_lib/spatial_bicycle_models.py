...
class SpatialBicycleModel(ABC):
    ...
    def get_current_waypoint(self, current_pose):
        """
        Get closest waypoint on reference path based on car's current location.
        """
        minID = None
        minDist = 1000
        for i in range(len(self.reference_path.waypoints)):
            dx = self.reference_path.waypoints[i].x - current_pose[0]
            dy = self.reference_path.waypoints[i].y - current_pose[1]
            dist = np.hypot(dx, dy)
            if dist < minDist:
                minDist = dist
                minID = i
        if minID is not None:
            self.current_waypoint = self.reference_path.waypoints[minID]
            self.wp_id = minID
        # # Original function
